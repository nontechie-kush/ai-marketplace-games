...
export async function proposeSpecPatch({ spec, userPrompt, briefSummary }) {
  ...
  // If the user clearly describes a bubble pop/clicker game, we still consult the LLM,
  // but if it returns nothing we'll fall back to a deterministic patch so the UX is never empty.
...
`BRIEF SUMMARY (<=300 chars):\n${shortSummary}\n\nCURRENT SPEC_JSON:\n${JSON.stringify(spec)}\n\nUSER MESSAGE:\n${userPrompt}\n\nReturn ONLY a JSON array of RFC-6902 operations.\n` }
  ...
  try {
    ...
    // If LLM fails, fall back for bubble prompts
    patch = looksLikeBubble ? bubbleFallbackPatch(userPrompt) : [];
  }

  // If the patch is empty or invalid and user asked for bubbles, apply our deterministic baseline
