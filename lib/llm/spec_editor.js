...
function parseLimitFromPrompt(userPrompt = '') {
  const p = String(userPrompt).toLowerCase();
  // e.g., "game over at 100 bubbles", "when 120 bubbles are on screen, game is over"
  const m = p.match(/(?:at|when)\s+(\d+)\s+bubbles?\b[^.]*\b(on\s+screen|game\s+is\s+over|it's\s+game\s+over)/);
  if (m) {
    const n = parseInt(m[1], 10);
    if (Number.isFinite(n) && n > 0) {
      return { concurrent_bubbles: n, end_on_limit: true };
    }
  }
  // also handle simple forms like "hit 100 bubbles on screen"
  const m2 = p.match(/hit\s+(\d+)\s+bubbles?\s+on\s+screen/);
  if (m2) {
    const n = parseInt(m2[1], 10);
    if (Number.isFinite(n) && n > 0) {
      return { concurrent_bubbles: n, end_on_limit: true };
...
function overrideOpsFromPrompt(userPrompt = '') {
  const ops = [];
  // skin always allowed to add (new field); if it already exists most patch libs upsert on add
  const skin = detectSkin(userPrompt);
  if (skin) {
    ops.push({ op: 'add', path: '/meta/skin', value: skin });
  }

  // spawn: use replace so we definitely override defaults present in the base spec
  const spawn = parseSpawnFromPrompt(userPrompt);
  if (spawn) {
    ops.push({ op: 'replace', path: '/rules/spawn', value: spawn });
  }

  // limit: capture phrases like "game over at 100 bubbles" or "when 120 bubbles are on screen"
  const lim = parseLimitFromPrompt(userPrompt);
  if (lim) {
    ops.push({ op: 'replace', path: '/rules/limit', value: lim });
  }
...
- "when N bubbles are on screen" or "game over at N bubbles" => rules.limit.concurrent_bubbles=N, end_on_limit=true.
