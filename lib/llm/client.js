...
### STYLING & GUIDANCE 
- Prefer inline SVGs for characters/icons (e.g., chickens, enemies, collectibles) instead of plain circles.
- Use CSS gradients, drop-shadows, and subtle transforms to give depth and polish.
- Backgrounds should be soft gradients or patterned fills, not flat solid colors.
- Buttons and HUD cards should have rounded corners and light shadows.
- Keep everything inline (no external images or fonts).
...
export function buildSystemPrompt(ops = {}) {
  ...
  const lines = [
    ...
    allowCDNAssets ? [
      'You MAY load assets only from this whitelist of CDNs:',
      'fonts.googleapis.com, fonts.gstatic.com (fonts);',
      'cdn.jsdelivr.net, cdnjs.cloudflare.com, unpkg.com (icons/SVG/assets).',
      'Use at most one Google Font family and 1–2 weights.',
    ].join(' ') : '',
  ].filter(Boolean);
