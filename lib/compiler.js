...
function compileBubbleClicker(spec = {}) {
  ...
  // Spawn normalization (accept multiple field names & modes)
  const spawnCfg = spec?.rules?.spawn || {};
  const modeRaw = (spawnCfg.mode || '').toString().toLowerCase();
  const spawnMode = (modeRaw === 'linear' || modeRaw === 'fixed' || modeRaw === 'add' || modeRaw === 'additive')
    ? 'linear'
    : 'doubling';
  
  // Prefer explicit intervalMs, else intervalSeconds, else legacy window_ms, else 2000ms
  const intervalMs = Number.isFinite(spawnCfg.intervalMs)
    ? Math.max(50, Math.floor(spawnCfg.intervalMs))
    : Number.isFinite(spawnCfg.intervalSeconds)
      ? Math.max(1, Math.floor(spawnCfg.intervalSeconds)) * 1000
      : (Number.isFinite(spec?.rules?.spawn?.window_ms) ? spec.rules.spawn.window_ms : 2000);
  
  // Accept addPerTick or add_per_tick
  const addPerTick = Number.isFinite(spawnCfg.addPerTick)
    ? Math.max(0, Math.floor(spawnCfg.addPerTick))
    : Number.isFinite(spawnCfg.add_per_tick)
      ? Math.max(0, Math.floor(spawnCfg.add_per_tick))
      : 0;
...
(() => {
  const cfg = {
    ...
    spawn: { mode: '${escapeJs(spawnMode)}', intervalMs: ${Math.round(intervalMs)}, addPerTick: ${addPerTick} }
  };
  ...
  hudSpawn.textContent = (cfg.spawn.mode === 'linear')
    ? ('+' + cfg.spawn.addPerTick + '/' + (cfg.spawn.intervalMs/1000) + 's')
    : ('1/' + (cfg.spawn.intervalMs/1000) + 's');
  ...
  function update(dt){
    ...
    if (cfg.spawn.mode === 'linear') {
      ...
      hudSpawn.textContent = '+' + cfg.spawn.addPerTick + '/' + (interval/1000) + 's';
    } else {
